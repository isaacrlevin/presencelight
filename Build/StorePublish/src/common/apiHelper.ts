...
export function includePackagesInSubmission(givenPackages: string[], submissionPackages: { fileName: string; fileStatus: string; }[]): void
{
 ...
 */
function createZipStream(zip: { generateNodeStream: (arg0: { base64: boolean; compression: string; type: string; streamFiles: boolean; }) => NodeJS.ReadableStream; }): NodeJS.ReadableStream
{
 ...
 * @param blobUrl
 */
export async function persistZip(zip: { generateNodeStream: (arg0: { base64: boolean; compression: string; type: string; streamFiles: boolean; }) => NodeJS.ReadableStream; }, filePath: string, blobUrl: string)
{
...
function uploadZip(filePath: string, blobUrl: string): Q.Promise<any>
{
    ...
    var defer = Q.defer();
    blobService.createBlockBlobFromLocalFile(containerName, blobName, filePath, options, function (error: any) {
        if (!error) {
